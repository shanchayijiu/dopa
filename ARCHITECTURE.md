...
### 2.3 瞄准 Pipeline — `aim_pipeline.py` / `AimPipeline` 类

独立的瞄准逻辑模块，从 `core.py` 中解耦：
...
| 模块 | 职责 |
|------|------|
| `infer_class.py` | ONNX Runtime 推理引擎封装，支持延迟初始化、多设备选择 |
| `inference_engine.py` | TensorRT 推理引擎（CUDA Graph 加速） |
| `infer_function.py` | NMS 处理（v5/v8 格式）、框坐标转换、可视化绘制 |
| `v11onnx_support.py` | YOLOv11 模型发现、验证、运行时配置 |
| `decode_model.py` | 加密模型 (.ZTX) 解密 |

### 2.5 截图与输入层

| 模块 | 职责 |
|------|------|
| `screenshot_manager.py` | 统一截图接口：BetterCam / OBS / 采集卡，内存池复用 |
| `obs.py` | OBS UDP 视频流接收与解码 |
| `dhz.py` | DHZ Box UDP 命令通信 |
...
| 模块 | 职责 |
|------|------|
| `pid.py` | PID 控制器算法（单轴 `PID` + 双轴 `DualAxisPID`） |
| `bytetrack.py` | ByteTrack 多目标跟踪器（IoU + 匈牙利算法） |
| `profiler.py` | 帧级性能分析 (EMA 平滑统计) |
| `gui_handlers.py` | GUI 配置变更统一处理框架 |
| `function.py` | 通用工具（按键转换、配置读取、距离计算） |
| `server.py` | NiceGUI Web 控制面板 |

### 2.7 验证与配置层

| 模块 | 职责 |
|------|------|
| `buff.py` | 卡密验证系统（RC4 加密，已禁用） |
| `remote_config.py` | 远程配置获取（AES-CBC 加密，JWT 令牌认证） |
| `server_config.py` | 服务器连接参数 |
